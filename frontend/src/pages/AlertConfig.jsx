...
export default function AlertConfig() {
  ...
  const handleToggle = async () => {
    if (!alert) return;
    try {
      const { data } = await api.patch(`/alerts/${alert._id}/toggle`);
      setAlert(data);
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">{account.institutionName}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card space-y-5">
        ...
        <fieldset className="border border-gray-200 rounded-lg p-4 space-y-3">
          ...
          <p className="text-xs text-gray-400">
            Available placeholders: <code>{"<<Deposit Date>>"}</code>, <code>{"<<Deposit Amount>>"}</code>, <code>{"<<Deposit Description>>"}</code>
          </p>
