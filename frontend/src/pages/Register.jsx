...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await register({ fname, lname, email, cell, password });
      navigate('/dashboard');
    } catch (err) {
      const details = err.response?.data?.details;
      setError(
        details?.[0]?.msg || err.response?.data?.error || 'Registration failed.'
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm">
        <h1 className="text-3xl font-bold text-brand-600 text-center mb-2">ClaimYourAid Admin</h1>
        ...
        <form onSubmit={handleSubmit} className="card space-y-4">
          ...
          <div>
            ...
            <input name="cell" type="text" required
              value={form.cell} onChange={handleChange}
